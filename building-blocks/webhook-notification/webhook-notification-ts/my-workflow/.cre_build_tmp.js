...
var safeJsonStringify = (obj) => JSON.stringify(obj, (_, v) => typeof v === "bigint" ? v.toString() : v, 2);
function formatPrice(raw, decimals) {
  const scaled = formatUnits(raw, decimals);
  const num = parseFloat(scaled);
  return num.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
}
function readFeed(runtime2, evmClient, name, address) {
...
function onCron(runtime2, _payload) {
  const { feed, chainName } = runtime2.config;
  ...
  runtime2.log(`Sending ${runtime2.config.notificationType} notification | url=${runtime2.config.webhookUrl}`);
...
function initWorkflow(config) {
  ...
  return [
    cre.handler(cron.trigger({ schedule: config.schedule }), onCron)
  ];
}
async function main() {
