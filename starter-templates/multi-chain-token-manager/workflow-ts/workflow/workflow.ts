...
import { ProtocolSmartWallet } from '../contracts/evm/ts/generated/ProtocolSmartWallet'
...
const readCurrentLiquidityRate = (
	...
	const pool = new MockPool(evmClient, evmCfg.poolAddress as Address)
	const reserveData = pool.getReserveData(runtime, evmCfg.assetAddress as Hex)
...
const readBalanceInPool = (
	...
	const evmClient = getEvmClientForChain(evmCfg)
	const pool = new MockPool(evmClient, evmCfg.poolAddress as Address)
	return pool.balanceOf(
		runtime,
		evmCfg.protocolSmartWalletAddress as Address,
...
const buildPoolForChain = (
	...
	const currentLiquidityRate = readCurrentLiquidityRate(runtime, evmCfg)
	runtime.log(`Reserve data [${evmCfg.chainName}] currentLiquidityRate: ${currentLiquidityRate}`)

	const balanceInPool = readBalanceInPool(runtime, evmCfg)
...
const performRebalance = (
	...
	const evmClient = getEvmClientForChain(evmCfg)
	const wallet = new ProtocolSmartWallet(evmClient, evmCfg.protocolSmartWalletAddress as Address)

	const resp = wallet.writeReportFromWithdrawFromPoolAndDepositCrossChain(
		runtime,
		{
...
const doHighestSupplyAPY = (runtime: Runtime<Config>): string => {
	...
	for (const evmCfg of config.evms) {
		...
		const balance = readBalanceInPool(runtime, evmCfg)
