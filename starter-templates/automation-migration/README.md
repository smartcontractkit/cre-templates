...
> **Disabling is not retroactive:** turning the check off resets the floor to `0`, so out-of-order reports start executing again going forward — but it does **not** revive any report that was already skipped as stale while the check was enabled. A below-floor report is consumed the moment `StaleReportSkipped` is emitted; the CRE Forwarder does not hold it for redelivery, so re-enabling fire-and-forget behavior only affects reports the workflow produces after you disable the check.
...
- **Stale report** — when the block-number monotonicity check is enabled for the pair (Step 3e) and the report's block number is older than the last accepted one — does **not** revert. The receiver emits `StaleReportSkipped(target, selector, reportBlockNumber, lastReportBlock)` and consumes the report without executing the call. Retrying would not help (the report is permanently stale), so it is skipped rather than marked retryable.
- **Gas guard failure** — when `setConsumerGasLimit` has been configured for the specific `(target, selector)` pair and the incoming gas is below `consumerGasLimit + consumerGasLimit / 63 + 7,000`, `_processReport` **reverts** with `InsufficientGas(available, required)`. The forwarder records the transmission as failed and it can be retried with higher gas. This closes a griefing attack where a report is delivered with just enough gas to pass the forwarder's minimum check but not enough for `performUpkeep` to execute, which would otherwise permanently consume the transmission ID. The 7,000-gas constant covers the post-guard path from the `gasleft()` check through function return: a **cold** `CALL` to the target (in production `setCallAllowed` runs in an earlier transaction, so the target account has not yet been touched when the report is delivered and the `CALL` pays the full EIP-2929 cold-access surcharge, not a warm rate), post-call `CallExecuted`/`CallFailed` LOG3 emission (3 topics), bookkeeping, and conservative headroom for future opcode repricing. The `consumerGasLimit / 63` term compensates for the EIP-150 (63/64) rule: a `CALL` can forward at most 63/64 of available gas, so without this buffer a high gas limit (above ~441,000) would cause the target to receive less than configured. Each `(target, selector)` pair has its own independent limit; pairs with no configured limit retain fire-and-forget semantics.
...
### Gas Limit
`writeGasLimit` (default `"500000"`) caps the on-chain execution gas forwarded to `onReport`. The on-chain guard formula is `consumerGasLimit + consumerGasLimit/63 + 7,000`, but an additional ~17,500 gas is consumed before that check point on the **first** delivery to a `(target, selector)` pair — four cold `SLOAD`s in `ReceiverTemplate.onReport` when identity checks are active (forwarder address, workflow ID, owner, name: 4 × 2,100 = 8,400 gas), warm re-reads of those slots in `_processReport` (~400 gas), `abi.decode` of the report payload (~300 gas), the cold nested-mapping read of `s_callAllowed` (~4,200 gas), the cold `s_consumerGasLimit` read (~2,100 gas), and `paused()` (~2,100 gas). After slots warm, pre-guard overhead drops to ~5,500 gas. When the block-number monotonicity check is enabled for a pair (Step 3e), budget a further ~10,000 gas for its two cold `SLOAD`s and the advancing `SSTORE`. Set `writeGasLimit` to at least your `performGasLimit` plus ~30,000 (or ~40,000 with the monotonicity check enabled) to ensure first deliveries comfortably clear the guard. If deliveries still fail with `InsufficientGas`, increase `writeGasLimit` in 10,000-gas increments until they pass.
